        ...
        <div class="card border-light shadow-sm components-section">
            <div class="card-body">
                ...
                <form>
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            <select class="form-select" id="programa_presupuestario" aria-label="Programas Presupuestarios" required>
                                ...
                                <option value="<?= $programa->programa_presupuestario_id ?>" data-descripcion="<?= $programa->descripcion ?>" data-objetivo="<?= $programa->objetivo ?>" <?= ($encabezado->programa_presupuestario_id == $programa->programa_presupuestario_id)? 'selected' : '' ?>>(<?= $programa->cve_programa ?>) <?= $programa->nombre ?></option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            <select class="form-select" id="linea_accion" aria-label="Líneas de Acción" required>
                                ...
                                <option value="<?= $linea->linea_accion_id ?>" data-objetivo="<?= $linea->objetivo_programa ?>" data-estrategia="<?= $linea->estrategia_programa ?>" <?= ($encabezado->linea_accion_id == $linea->linea_accion_id)? 'selected' : '' ?>><?= $linea->linea_accion ?></option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <select class="form-select" id="fuente_financiamiento" aria-label="Fuente de Financiamiento" required>
                                ...
                                <option value="<?= $fuente->fuente_financiamiento_id ?>" <?= ($encabezado->fuente_financiamiento_id == $fuente->fuente_financiamiento_id)? 'selected' : '' ?>><?= $fuente->descripcion ?></option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            <div>
                                ...
                                <textarea class="form-control" placeholder="¿Que actividades se desempeñaran?" id="detalle_actividad" rows="4"><?= $encabezado->actividad_general ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="unidad_medida" aria-label="Default select example">
                                ...
                                <option value="<?= $um->unidad_medida_id ?>" <?= ($encabezado->unidad_medida_id == $um->unidad_medida_id)? 'selected' : '' ?>><?= $um->descripcion ?> (<?= $um->cve_medida ?>)</option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="tipo_medicion" aria-label="Default select example">
                                ...
                                <option value="<?= $medicion->medicion_id ?>" <?= ($encabezado->medicion_id == $medicion->medicion_id)? 'selected' : '' ?>><?= $medicion->descripcion ?></option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="grupo_beneficiado" aria-label="Default select example">
                                ...
                                <option value="<?= $grupo->beneficiado_id ?>" <?= ($encabezado->beneficiado_id == $grupo->beneficiado_id)? 'selected' : '' ?>><?= $grupo->descripcion ?></option>
                                <?php endforeach; ?>  
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div id="programados" class="card card-body my-3 mx-2">
...
<script type="text/javascript">
    var pFisico     = {<?PHP foreach ($detalles as $key => $programado) { print( $programado->mes .' : "'.  $programado->programado_fisico     .'",'); } ?>},
        pFinanciero = {<?PHP foreach ($detalles as $key => $programado) { print( $programado->mes .' : "'.  $programado->programado_financiero .'",'); } ?>},
        tFisico     = <?= $encabezado->programado_fisico ?>,
        tFinanciero = <?= $encabezado->programado_financiero ?>,
        inputs      = JSON.parse('<?php print(json_encode($inputs, JSON_HEX_TAG)); ?>');
</script>
