...
function finicia_distribucion(){
    meses = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
    meses.forEach( function(mes, index) {
        $(`#fisico_${mes}`).val(pFisico[index+1]);
        $(`#financiero_${mes}`).val(pFinanciero[index+1]);
    });
    $('#fisico_objetivo_anual').val(tFisico);
    $('#financiero_objetivo_anual').val(tFinanciero);
}
...
function fguardar(e){
    ...
    try {
        ...
        if ( ! errores ){
            datos['proyecto_id']    = $('#proyecto_id').val();
            datos['actividad_id']   = $('#actividad_id').val();
            respuesta   = fu_json_query(
                url('Actividades/guardar_edicion', true, false),
                datos 
            );
            if ( respuesta.exito ){
